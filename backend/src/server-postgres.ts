...
const optionalAuth = (req: express.Request, res: express.Response, next: express.NextFunction) => {
  ...
  const deviceToken = (req.headers['x-client-token'] as string | undefined) || (req.headers['x-device-token'] as string | undefined);
  ...
  (async () => {
    if (deviceToken && !(req as any).user) {
      try {
        const result = await db.verifyDeviceToken(deviceToken);
        if (result) {
          (req as any).user = { userId: result.user_id, email: 'device@client' } as JWTPayload;
        }
      } catch {
        // ignore
      }
...
app.get('/api/pairing/code', authenticateToken, async (req, res) => {
  try {
    const payload = (req as any).user as JWTPayload;
    // Try to get an existing code; if none exists, create one on-demand
    let result = await db.getActiveDeviceCode(payload.userId);
    if (!result) {
      try {
        const created = await db.getOrCreateDeviceCode(payload.userId);
        result = created;
      } catch (e) {
        console.warn('Auto-creating pairing code failed:', e);
      }
    }

    if (!result) {
      return res.status(500).json({ error: 'Failed to provide pairing code' });
    }
