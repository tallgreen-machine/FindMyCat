...
exports.pool = exports.PostgresDatabase = void 0;
...
class PostgresDatabase {
    ...
    async createUser(email, password, displayName) {
        ...
        try {
         ...
         VALUES ($1, $2, $3)
         RETURNING id, email, display_name, is_active, created_at`, [email.toLowerCase().trim(), passwordHash, displayName?.trim()]);
    ...
    async authenticateUser(email, password) {
        const client = await this.connect();
        try {
            const result = await client.query(`SELECT id, email, password_hash, display_name, is_active, created_at
         FROM users 
         WHERE email = $1 AND is_active = true`, [email.toLowerCase().trim()]);
    ...
    async getUserById(userId) {
        const client = await this.connect();
        try {
            const result = await client.query(`SELECT id, email, display_name, is_active, created_at
         FROM users 
    ...
    async getOrCreateDeviceCode(userId) {
        const client = await this.connect();
        try {
            // Try to find an existing pairing code for this user
            const existing = await client.query(`SELECT code FROM pairing_codes
         WHERE user_id = $1
         LIMIT 1`, [userId]);
            ...
            const result = await client.query(`INSERT INTO pairing_codes (code, user_id)
         VALUES ($1, $2)
         RETURNING code`, [code, userId]);
    ...
    async getActiveDeviceCode(userId) {
        const client = await this.connect();
        try {
            const existing = await client.query(`SELECT code FROM pairing_codes WHERE user_id = $1 LIMIT 1`, [userId]);
    ...
    async useDeviceCode(code) {
        const client = await this.connect();
        try {
            const codeResult = await client.query(`SELECT user_id FROM pairing_codes 
         WHERE code = $1`, [code.toUpperCase().trim()]);
    ...
    async registerDevice(userId, deviceId, name) {
        const client = await this.connect();
        try {
            const result = await client.query(`INSERT INTO devices (device_id, user_id, name, last_seen)
         ...
         DO UPDATE SET 
           ...
           is_active = true
         RETURNING *`, [deviceId, userId, name]);
            return result.rows[0];
        }
        finally {
            client.release();
    ...
    async updateDevice(userId, deviceId, updates) {
        ...
        try {
            ...
            const result = await client.query(`UPDATE devices 
    ...
    // Location management
    async addLocation(userId, deviceId, location) {
        const client = await this.connect();
        try {
         ...
         WHERE user_id = $1 AND device_id = $2`, [userId, deviceId, location.timestamp]);
            // Insert location
            const result = await client.query(`INSERT INTO locations (device_id, user_id, latitude, longitude, accuracy, altitude, speed, heading, timestamp)
    ...
    async getLatestLocations(userId, deviceId) {
        const client = await this.connect();
        try {
            if (deviceId) {
           ...
           LIMIT 1`, [userId, deviceId]);
                ...
                const result = await client.query(`SELECT DISTINCT ON (device_id) *
           FROM locations 
           WHERE user_id = $1
           ORDER BY device_id, timestamp DESC`, [userId]);
                return result.rows;
            }
        }
        finally {
    ...
    async getLocationHistory(userId, deviceId, limit = 1000) {
        const client = await this.connect();
        try {
            if (deviceId) {
                const result = await client.query(`SELECT * FROM locations 
    ...
    generateRawDeviceToken() {
        // 32 bytes random -> 64 hex chars
        return crypto_1.default.randomBytes(32).toString('hex');
    }
    async createDeviceToken(userId, name) {
        const client = await this.connect();
        try {
            const raw = this.generateRawDeviceToken();
            const tokenHash = this.hashToken(raw);
            const result = await client.query(`INSERT INTO client_tokens (user_id, name, token_hash)
         VALUES ($1, $2, $3)
         RETURNING *`, [userId, name || null, tokenHash]);
    ...
    async verifyDeviceToken(rawToken) {
        const client = await this.connect();
        try {
            const tokenHash = this.hashToken(rawToken);
            const result = await client.query(`SELECT user_id FROM client_tokens
    ...
    // Migration helpers
    async migrateFromSQLite(sqliteLocations) {
        const client = await this.connect();
        ...
        try {
            ...
            for (const loc of sqliteLocations) {
                ...
                // Add location
                await client.query(`INSERT INTO locations (device_id, user_id, latitude, longitude, timestamp)
           VALUES ($1, $2, $3, $4, $5)
           ON CONFLICT (device_id, user_id, timestamp) DO NOTHING`, [loc.deviceId, demoUserId, loc.latitude, loc.longitude, loc.timestamp]);
            }
            await client.query('COMMIT');
        }
        catch (error) {
